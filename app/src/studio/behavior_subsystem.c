...
static bool encode_value_description(pb_ostream_t *stream, const pb_field_t *field,
    ...
    bool param1 = field->tag == zmk_behaviors_BehaviorBindingParametersSet_param1_tag;
    size_t values_len = param1 ? set->param1_values_len : set->param2_values_len;
    const struct behavior_parameter_value_metadata *values =
        param1 ? set->param1_values : set->param2_values;
    ...
    for (int val_i = 0; val_i < values_len; val_i++) {
        ...
        case BEHAVIOR_PARAMETER_VALUE_TYPE_HID_USAGE:
            ...
            desc.value_type.hid_usage.consumer_max = 0xFF;
#else
            desc.value_type.hid_usage.consumer_max = 0xFFF;
#endif

#if IS_ENABLED(CONFIG_ZMK_HID_REPORT_TYPE_NKRO)
            desc.value_type.hid_usage.keyboard_max = CONFIG_ZMK_HID_REPORT_TYPE_NKRO;
#else
            desc.value_type.hid_usage.keyboard_max = 0xFF;
#endif
            break;
