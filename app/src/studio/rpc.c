...
static zmk_Response handle_request(const zmk_Request *req) {
...
static bool rpc_tx_buffer_write(pb_ostream_t *stream, const uint8_t *buf, size_t count) {
    ...
    do {
        ...
        uint8_t *write_buf;
        uint8_t claim_len = ring_buf_put_claim(&rpc_tx_buf, &write_buf, count - written);

        if (claim_len == 0) {
            continue;
        }

        int8_t escapes_written = 0;
        for (int i = 0; i < claim_len && write_idx < claim_len; i++) {
            ...
            case FRAMING_SOF:
                ...
                // if the escape has already bee written in the previous iteration
                // of our loop.
...
static int send_response(const zmk_Response *resp) {
...
static void rpc_main(void) {
    for (;;) {
        ...
        if (status) {
            zmk_Response resp = handle_request(&req);
...
static int studio_rpc_listener_cb(const zmk_event_t *eh) {
    ...
    zmk_Notification n = zmk_Notification_init_zero;
